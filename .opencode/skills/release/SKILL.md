...
5. **Build differentiel** : lancer `deploy\build-diff.bat` (ou `powershell -NoProfile -ExecutionPolicy Bypass -File deploy\_build.ps1 -Diff`)
   ...
   - le tag est detecte dynamiquement (`git describe --tags --abbrev=0`) ; le build complet reste `deploy\build.bat`
